...
async def start(message: types.Message):
    await message.reply('используйте /search <запрос>, для поиска вакансий.\nдля остального функционала используйте /help')

@dp.message_handler(commands=['help'])
async def start(message: types.Message):
    await message.reply('краткая сводка по командам\n/start - запуск бота\n/search <запрос> - поиск вакансий по запросу\n/recent - вывод 5 рандомных вакансий\n/count - вывод общего кол-ва вакансий в базе данных\n/grafic - вывод на выбор режима рабочего дня\n/search_company - поиск вакансий по компании из базы данных\n/search_vacancy - поиск вакансий по названию вакансии из базы данных')
...
async def search_by_company(message: types.Message):
    ...
    if not company_name:
        await message.reply('введите название компании после команды /search_company.')
        return
...
async def search_by_vacancy(message: types.Message):
    ...
    if not vacancy_query:
        await message.reply('введите название вакансии после команды /search_vacancy.')
        return
